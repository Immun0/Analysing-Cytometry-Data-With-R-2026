...
```{r run-umap-lineage, warning=FALSE}
set.seed(1234)
...
```{r plot-umap-lineage, warning=FALSE}
...
```{r run-tsne-lineage, warning=FALSE}
...
```{r plot-tsne-lineage, warning=FALSE}
...
```{r umap-seed-stability, warning=FALSE}
...
```{r umap-what-separates, warning=FALSE}
...
```{r umap-every-marker, warning=FALSE}
marker_plots <- list()
...
```{r umap-marker-examples, warning=FALSE}
...
```{r umap-every-marker-pdf, warning=FALSE}
...
`gg_save_pdf()` is worth keeping. `ggsave()` writes one plot to one file, so a 37-panel survey would be ten separate files to open and compare. Opening a PDF device, printing into it repeatedly and closing it puts everything in one document you can page through, which is how you actually read a survey like this.

**A warning on file size.** That PDF is around 55 MB, because a PDF stores every one of the 25,000 points on every one of the 37 panels as its own vector object. If you need to email it or put it in a repository, either write a PNG instead, at the cost of not being able to zoom, or downsample with `sample_frac()` first as described further down. It is also worth checking that your `.gitignore` covers PDFs; this project's currently does not.
...
Worth checking rather than assuming, though, and it is one line. `table()` on the top 5% of a marker tells you whether its high cells belong to one condition or to all of them:

```{r umap-marker-condition-check}
...
```{r umap-gate-regions, warning=FALSE}
...
```{r umap-gate-manual, warning=FALSE}
...
Two habits worth taking from this. Get the coordinates from a printed table, not from squinting at the plot. And check the gate's contents afterwards, because a gate that looks right and a gate that is right are different things, and the `table()` above is the difference between the two.
...
```{r umap-territories, warning=FALSE}
...
```{r umap-by-condition, warning=FALSE}
...
```{r umap-per-sample, warning=FALSE}
...
```{r umap-per-sample-nested, eval=FALSE}
library(ggh4x)
...
```{r umap-composite, warning=FALSE}
title <- ggdraw() +
...
```{r umap-animate-show, echo=FALSE}
knitr::include_graphics(here::here("Images", "umap_animation_cumulative.gif"))
```
...
```{r interactive-marker-choice, eval=FALSE}
library(tcltk)
...
```{r umap-chosen-markers, warning=FALSE}
