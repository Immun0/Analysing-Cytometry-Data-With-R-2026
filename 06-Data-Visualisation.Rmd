...
```{r different-markers, eval=FALSE}
autoplot(FLOWSET, x = "Pr141Di", y = "Nd142Di")
```

**Check available channels:**
```{r check-markers, eval=FALSE}
colnames(FLOWSET)
```
...
**Plot one sample:**
```{r one-sample-plot, eval=FALSE}
autoplot(FLOWSET[[1]], x = "Ir191Di", y = "Ir193Di")
```

**Plot all samples:**
```{r all-samples-plot, eval=FALSE}
...
```{r overlay-histogram, eval=FALSE}
library(ggcyto)
...
```{r check-all-markers, eval=FALSE}
# Plot all markers
autoplot(FLOWSET[[1]])
```

**Compare before and after processing:**
```{r compare-processing, eval=FALSE}
...
```{r size-params, eval=FALSE}
autoplot(FLOWSET, x = "Event_length", y = "Width", bins = 128)
```

**Channel expression:**
```{r marker-expression, eval=FALSE}
