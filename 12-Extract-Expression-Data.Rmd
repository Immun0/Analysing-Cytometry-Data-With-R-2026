...
The check worth running is not "are these in the same order" but "are these the same five samples at all":
...
It is worth seeing this fail rather than taking the warning on trust, because the failure does not look like a failure. Build the labels both ways on the real data and compare.
