...
```{r peacoqc-cleaning, warning=FALSE, message=FALSE}
...
```{r compare-before-after, warning=FALSE}
library(cowplot)
library(ggcyto)
...
```{r compare-counts, warning=FALSE}
library(ggplot2)
...
```{r flowCut-cleaning, warning=FALSE, message=FALSE}
...
```{r flowcut-alwaysclean, eval=FALSE}
CLEANED_AC <- fsApply(
  Reordered_RENAMED_FLOWSET,
  function(x) flowCut(
    x,
